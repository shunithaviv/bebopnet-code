...
class ResultsLog(object):
    ...
    def __init__(self, path='', title='', params=None, resume=False, data_format='csv', epochs=0, index='epoch'):
        """
        Parameters
        ----------
        path: string
    ...
    def add(self, **kwargs):
        ...
        example:
            resultsLog.add(epoch=epoch_num, train_loss=loss,
                           test_loss=test_loss)
        """
        df = pd.DataFrame([kwargs.values()], columns=kwargs.keys()).set_index(self.index)
        self.results = pd.concat((self.results, df), sort=False)
    ...
    def plot(self, x, y, title=None, xlabel=None, ylabel=None, legend=None,
             width=800, height=400, line_width=2, avg=False,
        ...
        for i, yi in enumerate(y):
            if avg:
                columns = [key for key in self.results.keys() if key.endswith(yi)]
                tmp_res = self.results[columns]
                for col in columns:
                    tmp_res[col] = tmp_res[col].shift(-(self.epochs + 1) * int(col[1]))
                plot_y = tmp_res.mean(axis=1)
            else:
                plot_y = self.results[yi]
            f.line(self.results.index, plot_y,
